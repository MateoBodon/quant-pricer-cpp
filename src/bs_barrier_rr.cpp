...
struct RrContext {
    ...
    RrContext(double S_, double K_, double B_, double r_, double q_, double sigma_, double T_, double rebate_)
        : S(S_), K(K_), B(B_), r(r_), q(q_), sigma(sigma_), T(T_),
          disc_r(std::exp(-r_ * T_)),
          disc_q(std::exp(-q_ * T_)),
          sigma_sqrt_T(sigma_ * std::sqrt(T_)),
          mu((r_ - q_ - 0.5 * sigma_ * sigma_) / (sigma_ * sigma_)),
          mu_sigma((1.0 + mu) * sigma_sqrt_T),
          lambda(std::sqrt(mu * mu + 2.0 * (r_ - q_) / (sigma_ * sigma_))),
          rebate(rebate_) {}
