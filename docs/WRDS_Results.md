...
**Local WRDS raw data (real data):** Local mode is **explicit-only and fail-closed**. Set `WRDS_LOCAL_ROOT` (env) or `wrds_local_root` in the dateset config to use the partitioned OptionMetrics CSV.GZ vault bound to snapshot `20260707_045553_global_project_priority`. The adapter reads exact `opprcdYYYY` day and `secprdYYYY` month partitions plus `secnmd`, `zerocd`, and `idxdvd`; it resolves SPX as of each date, interpolates the exact-date zero curve, and refuses missing or ambiguous spot/rate/dividend inputs. An explicit local failure never falls back to constants, cache, live WRDS, or sample data. Local runs default to `artifacts/_local/wrds_local/` (scratch), with `source_receipt.json` and `manifest_local.json` kept ignored so restricted paths and identifiers never enter Git.
...
| --- | --- | --- | --- |
| In-sample vega-wtd IV RMSE | 0.0160 | vol pts | [`wrds_agg_pricing.csv`](artifacts/wrds/wrds_agg_pricing.csv) |
| In-sample vega-wtd IV MAE | 0.0128 | vol pts | [`wrds_agg_pricing.csv`](artifacts/wrds/wrds_agg_pricing.csv) |
| In-sample 90th pct IV error | 227.0 | bps | [`wrds_agg_pricing.csv`](artifacts/wrds/wrds_agg_pricing.csv) |
| In-sample price RMSE | 186.54 | ticks | [`wrds_agg_pricing.csv`](artifacts/wrds/wrds_agg_pricing.csv) |
| Next-day IV MAE (quotes/vega-wtd) | 127.29 | bps | [`wrds_agg_oos.csv`](artifacts/wrds/wrds_agg_oos.csv) |
| Next-day price MAE (quotes/vega-wtd) | 137.26 | ticks | [`wrds_agg_oos.csv`](artifacts/wrds/wrds_agg_oos.csv) |
| Delta-hedged mean ticks (30/60/90d) | -79.6 / -72.3 / -67.2 | ticks | [`wrds_agg_pnl.csv`](artifacts/wrds/wrds_agg_pnl.csv) |
| Delta-hedged sigma ticks (30/60/90d) | 91.8 / 62.7 / 46.1 | ticks | [`wrds_agg_pnl.csv`](artifacts/wrds/wrds_agg_pnl.csv) |

## BS baseline posture

For comparison, the tracked curated sample artifact is `docs/artifacts/wrds/wrds_bs_heston_comparison.csv`. Some sample runs also generate BS detail files such as `wrds_agg_pricing_bs.csv` and `wrds_agg_oos_bs.csv`, but those files are ignored by the current tracking policy and must not be treated as public curated evidence unless a later ticket deliberately promotes safe aggregates.
...
Live/local WRDS evidence is not current public truth by default. Historical local/live runs may exist in run logs or scratch directories, but they require a locked protocol, sanitized aggregate artifacts, data-policy verification, and Heavy/Pro L4 review before promotion. Do not use the deterministic sample panel above as evidence of live-market performance or Heston superiority.

Future live/local runs should write raw or detailed outputs to ignored scratch paths first, then promote only reviewed aggregate artifacts.
