...
def main():
    ...
    ap.add_argument(
        "--min_moneyness", type=float, default=0.6, help="Minimum K/S to retain"
    )
    ap.add_argument(
        "--max_moneyness", type=float, default=1.4, help="Maximum K/S to retain"
    )
    ap.add_argument(
        "--min_days", type=int, default=2, help="Minimum calendar days to expiry"
    )
    args = ap.parse_args()
    ...
    try:
        ...
        if not closes:
            print(
                "No underlying close prices for selected secids/date", file=sys.stderr
            )
            sys.exit(2)
    ...
    if df is None or df.empty:
        print(
            "WRDS query failed or returned no rows: {}".format(last_err),
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    mid = np.where(
        bb.notna() & bo.notna(),
        0.5 * (bb.to_numpy() + bo.to_numpy()),
        np.where(
            bb.notna(), bb.to_numpy(), np.where(bo.notna(), bo.to_numpy(), np.nan)
        ),
    )
    df["mid"] = mid
    ...
    if under_candidates:
        ...
        if "under_price" in df.columns:
            ...
            print(
                "Could not locate under_price column in security price table",
                file=sys.stderr,
            )
            sys.exit(4)
    ...
    df = df[
        (df["moneyness"] >= args.min_moneyness)
        & (df["moneyness"] <= args.max_moneyness)
    ]
