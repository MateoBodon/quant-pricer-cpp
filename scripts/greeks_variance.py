...
def main():
    ...
    for paths in grid:
        mp = qp.McParams()
        mp.spot=S; mp.strike=K; mp.rate=r; mp.dividend=q; mp.vol=sig; mp.time=T
        mp.num_paths=paths; mp.seed=args.seed; mp.antithetic=True; mp.control_variate=False
    ...
    with open(csv_path, 'w', newline='') as fh:
        writer = csv.DictWriter(fh, fieldnames=['paths','gamma_lrm_se','gamma_mixed_se'])
        writer.writeheader()
        for row in rows:
            writer.writerow(row)
