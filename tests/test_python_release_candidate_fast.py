#!/usr/bin/env python3
"""Focused consistency checks for the v0.3.5 Python release candidate."""
...
class PythonReleaseCandidateTest(unittest.TestCase):
    def test_authoritative_version_surfaces_match(self) -> None:
        ...
        self.assertRegex(pyproject, r'(?m)^version = "0\.3\.5"$')
        self.assertRegex(
            cmake, r"(?m)^project\(quant_pricer_cpp VERSION 0\.3\.5 LANGUAGES CXX\)$"
        )
        self.assertRegex(header, r"(?m)^constexpr int kVersionMajor = 0;$")
        self.assertRegex(header, r"(?m)^constexpr int kVersionMinor = 3;$")
        self.assertRegex(header, r"(?m)^constexpr int kVersionPatch = 5;$")
