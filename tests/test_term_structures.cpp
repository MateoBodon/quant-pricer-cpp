...
TEST(TermStructures, McConstantScheduleMatchesScalar) {
    ...
    a.rate_schedule = r; a.dividend_schedule = q; a.vol_schedule = v;
    auto sch = quant::mc::price_european_call(a);
    EXPECT_NEAR(base.estimate.value, sch.estimate.value, 3.0 * sch.estimate.std_error + 3.0 * base.estimate.std_error);
}

TEST(TermStructures, PdeConstantScheduleMatchesScalar) {
    quant::pde::PdeParams p{
        .spot=100.0, .strike=100.0, .rate=0.02, .dividend=0.01, .vol=0.20, .time=1.0,
        .type=quant::OptionType::Call, .grid=quant::pde::GridSpec{241, 240, 4.0, 2.0},
        .log_space=true, .upper_boundary=quant::pde::PdeParams::UpperBoundary::Neumann,
        .compute_theta=false, .use_rannacher=true
    };
    ...
    quant::PiecewiseConstant q{{0.25, 0.50, 1.0}, {0.01, 0.01, 0.01}};
    quant::PiecewiseConstant v{{0.25, 0.50, 1.0}, {0.20, 0.20, 0.20}};
    p.rate_schedule = r; p.dividend_schedule = q; p.vol_schedule = v;
...
TEST(TermStructures, McGreeksConstantScheduleMatchesScalar) {
    ...
    const double delta_ref = quant::bs::delta_call(base.spot, base.strike, base.rate, base.dividend, base.vol, base.time);
    const double vega_ref = quant::bs::vega(base.spot, base.strike, base.rate, base.dividend, base.vol, base.time);
    const double gamma_ref = quant::bs::gamma(base.spot, base.strike, base.rate, base.dividend, base.vol, base.time);
