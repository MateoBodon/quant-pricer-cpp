...
- Live/local WRDS outputs must not be committed; keep them outside the repo or under gitignored paths such as `artifacts/_local/` (data-policy guard enforces restricted-column rules).
- Local WRDS runs should avoid touching `docs/artifacts/manifest.json`; store local provenance in `artifacts/_local/wrds_local/manifest_local.json` (ignored).
